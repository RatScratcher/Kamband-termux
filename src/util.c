...
#if defined(MACINTOSH) || (defined(WINDOWS) && !defined(__MINGW32__))
...
/*
 * Recall the priority of a saved message
 */
byte message_prior(s16b age)
{
	s16b x;
	...
	return message__pty[x];
}

/*
 * Add a new message, with great efficiency
 */
void message_add(cptr str, byte prior)
{
	int i, k, x, n;
...
void mprint(byte priority, cptr msg)
{
	...
	/* Color */
	color = (priority == MSG_TEMP ? TERM_WHITE : priority);
...
void msg_format(cptr fmt, ...)
{
	...
	mprint(MSG_NORMAL, buf);
}

void mformat(byte prior, cptr fmt, ...)
{
	va_list vp;
 ...
 */
bool askfor_aux(char *buf, int len, bool complete_cli)
{
	int y, x;
...
/*
 * Like ``get_check'', but return value of -1 to mean ``cancel''.
 */
int get_three_way_check(cptr prompt)
{
	int i;
	...
	/* Hack -- Build a "useful" prompt */
	strnfmt(buf, 78, "%.70s[y/n/ESC] ", prompt);
